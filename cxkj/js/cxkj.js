...
_3DCarousel.prototype = {
  init: function (element) {
    ...
    this.content.onmouseover = function () {
      if (oThis.timer !== null) {    // 鼠标移入，轮播图停止
        clearInterval(oThis.timer);
        oThis.timer = null;
      }

      oThis.dirl.onclick = function () {
		oThis.change();    //点击向左按钮，轮播图左移
	  }
      oThis.dirr.onclick = function () {
		oThis.change(-1);      //点击向右按钮，轮播图右移
	  }
    };

    this.content.onmouseout = function () {
      if (oThis.timer === null) {    // 只有在没有自动轮播的情况下才会触发自动轮播
        oThis.timer = oThis.auto();
      }
    };

    oThis.timer = oThis.auto();     // 自动轮播
  },
  ...
  change: function (step) {   // 轮播图变换，默认从右向左移动；如果向右移动，则需要传入负数
